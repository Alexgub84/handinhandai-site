...
### [Bug] `overflow-x: hidden` on html/body silently broke `position: sticky` site-wide

**Date:** 2026-05-24
**Problem:** The cinematic homepage relies on `position: sticky` stages (hero chaos→orb, and the new scroll-film) to pin a visual while scroll scrubs it. None of them actually pinned — the "pinned" stage scrolled off-screen with the page. The hero looked plausible on first load so it went unnoticed, but no chapter ever held in the viewport.
**Solution:** `html, body { overflow-x: hidden }` turns the root into a scroll container, and a sticky element resolves against the nearest scroll-container ancestor — so every page-level sticky stuck to that container instead of the viewport and tracked the scroll. Changed to `overflow-x: clip`, which clips horizontal overflow WITHOUT establishing a scroll container. Sticky immediately started pinning for both the hero and the film.
**Prevention:** When you need to suppress horizontal scroll on a page that uses page-level `position: sticky`, use `overflow-x: clip`, never `overflow-x: hidden`. If a sticky element won't pin, the first suspect is a non-`visible` `overflow` on any ancestor (most often the root).

### [Decision] Homepage = one pinned scroll-scrub film, not a march of sticky text slides

**Date:** 2026-05-24
**Context:** The homepage felt like "scroll a lot": after the cinematic hero, chapters 2–7 were `position: sticky` full-screen text slides only one viewport tall each, so they didn't scrub — they marched past one static screen at a time, and the orb vanished after the hero. The reference the user wanted (playwithanimation.netlify.app) is one pinned visual where scroll changes the content in place.
**Decision:** Folded chapters 2–7 into a single `StoryFilm` component: one tall section (`min-height: 620dvh`) with one sticky stage, a persistent orb backdrop, and the chapter copy as absolutely-positioned layers that cross-fade by `[start,end)` scroll range on a single timeline (driven by the existing `sectionProgress`/`smoothstep` helpers). Vignette cards sub-sequence within the vignettes band. Retired `WorkflowVignettes` (folded in). Rejected: (a) true horizontal pin scroll — fights RTL and degrades on mobile; (b) just deleting chapters to shorten the page — loses content the copy needs. Rejected keeping per-chapter paper/gold backgrounds — they fight a persistent orb; unified the stage to dark.
**Reuse tip:** The pinned-scrub pattern is now used three places with the same shape (tall section + sticky stage + `sectionProgress` → toggle `.is-active` by range). To add a chapter, add a beat to `BEATS` in `StoryFilm.tsx` and a `.film-layer[data-chapter]` block. The stage exposes the active beat on `data-film-chapter` for tests/debugging.

### [Decision] Russian + Ukrainian as full `ui` blocks keyed to the Hebrew default, not partial overlays

**Date:** 2026-05-29
**Context:** A prior session wrote Russian strings into a broken helper script (`scripts/add-ru-uk-translations.js`) that was never applied, and had no Ukrainian at all. `useTranslations` falls back to `ui[defaultLang]` (Hebrew) for any missing key, and the translation key type is `keyof ui.he` — so a language object can technically be partial and still typecheck, but any gap renders Hebrew on a Russian/Ukrainian page.
**Decision:** Added complete `ru` and `uk` blocks to `src/i18n/ui.ts` covering the **full Hebrew keyset** (homepage + both fitness pages + the orphaned `[lang]` compatibility keys), not just nav/hero. Russian came from the helper script's `ruTranslations`; Ukrainian was written fresh (proper Ukrainian business tone — "Рука в руці…", "Поговорімо", "Написати Саші" — not transliterated Russian). `getRouteFromUrl` now strips any known non-default prefix (`en`/`ru`/`uk`) via a `knownLangs` list instead of hardcoding `en`, so the language switcher preserves the current route across all four languages. Deleted the helper scripts after applying. Rejected: running the script (it had placeholder logic and no Ukrainian); rejected partial blocks relying on Hebrew fallback (would mix RTL Hebrew into LTR pages).
**Reuse tip:** To add a fifth language, add it to `languages`, add it to `knownLangs` in `utils.ts`, add a full block to `ui` covering every Hebrew key, and confirm `getDirection` returns the right `dir` (only `he` is RTL). `npm run build` runs `astro check`, which is the typecheck gate — there is no separate `lint`/`typecheck` script in this project.

### [Win] LanguageSwitcher shows short codes on mobile, full labels from `sm:` up

**Date:** 2026-05-29
**Context:** With four languages the switcher pills render full native labels ("Українська" is 10 chars); next to the logo + hamburger they overflow a 320px header.
**Strategy:** Render two spans per pill — `<span class="sm:hidden">{code.toUpperCase()}</span>` and `<span class="hidden sm:inline">{label}</span>` — with `aria-label={label}` so the accessible name stays the full language name regardless of which span is visible.
**Why it works:** Tailwind's mobile-first breakpoints swap the visible text at the `sm` boundary without JS; the short code (HE/EN/RU/UK) fits four pills on the narrowest screens while desktop keeps the readable native label.

### [Decision] Ported the GSAP/React/Vite homepage design to static Astro with CSS-first animations

**Date:** 2026-06-15
**Context (trade-off):** The finished homepage design (`handinhand-production`) was a React 18 + Vite SPA driven by GSAP/ScrollTrigger + Lenis. Porting it verbatim as React islands would keep exact animations but ship a heavy JS bundle and (with `client:only`) hurt SEO/AI-findability; a one-to-one GSAP port also fights Astro's static-first model.
**Decision (chosen + rejected):** Rebuilt every section as static `.astro` components under `src/components/handinhand/` with the design's CSS copied verbatim into `src/styles/handinhand/{tokens,base,app}.css`, imported **only** in `index.astro` so they stay page-local (no leak into Tailwind pages). Animations were re-expressed CSS-first: hero word-mask + load fades as CSS keyframes; scroll-progress + the hub floats as CSS `animation-timeline: scroll()` / keyframes; integration-hub pulses as **SVG SMIL** (`animateMotion`/`animate`) instead of GSAP MotionPath; scroll reveals via one small IntersectionObserver toggling `.is-in`. A single bundled `<script>` in `index.astro` covers only what CSS can't: nav scrolled/active state, mobile menu, the timeline active-step, the floating-WhatsApp reveal, the free-text ChatDemo→`wa.me`, the cycling DiagChat, and a ~6-line ResizeObserver that scales the fixed 480×520 hub stage to its container. Dropped GSAP, Lenis, React, iconsax (icons re-drawn as inline SVG in `Icon.astro`), Magnetic and Counter (decorative). Rejected: React-island port (bundle weight + empty-root SEO cost) and a pure-CSS DiagChat loop (can't swap text content cleanly).
**Reason:** Static HTML is the biggest AI-findability win over the SPA's empty `#root`, and CSS-first keeps the page fast while preserving the animation feel; everything degrades gracefully (content visible without JS / under `prefers-reduced-motion`).
**Reuse tip:** When importing a global stylesheet that contains resets (`* { box-sizing }`, `body {}`), import it inside the single page that needs it — Astro scopes page-imported CSS to that page's bundle, so it won't collide with Tailwind on other routes. For scroll-position animations prefer CSS `animation-timeline`/SMIL before reaching for JS; reserve JS for state and content swaps. WhatsApp links live only in `src/data/site-contact.ts` (`waLink`/`whatsappCta`) — never hardcode `wa.me`.

### [Decision] Dropped Russian/Ukrainian; /en aliases the new Hebrew homepage

**Date:** 2026-06-15
**Context (trade-off):** After the homepage rebuild, `/en /ru /uk /he` still rendered the old SimpleLayout design. Requirement: `/` and `/en` should show the new homepage, no `/he` duplicate, and Russian/Ukrainian gone from the site.
**Decision (chosen + rejected):** Extracted the new homepage into a shared `src/components/handinhand/HomeDocument.astro` (full `<html>` document) rendered by both `src/pages/index.astro` (`/`) and `src/pages/[lang]/index.astro` whose `getStaticPaths` now returns only `{ lang: "en" }`. `/en` is identical Hebrew content with `canonical` → `/` to dedupe. Removed `ru`/`uk` from `languages` and deleted their `ui` blocks in `src/i18n/ui.ts`; trimmed `knownLangs` in `utils.ts` to `['en']`. `LanguageSwitcher` and the sitemap i18n config pick up the change automatically. Rejected: keeping `[lang]` homepage on the old design (inconsistent), and translating the new design to en (user wants one shared page).
**Reason:** One canonical homepage with an `/en` alias keeps inbound English links working without duplicate-content or a second translation to maintain.
**Reuse tip:** To make one route reuse another page verbatim, render a shared full-document component from both and point `canonical` at the primary URL. `src/pages/[lang]/index.astro` getStaticPaths controls exactly which locale homepages exist — Hebrew stays at `/` (no `/he`).
